...
export default function CanMap({ selectedStates = [], onStateSelect, choroplethData, choroplethDimension }) {
        ...
        if (svgRef.current) {
            ...
            svg.selectAll(".state")
                .classed("is-selected", d => {
                    const name = d.properties.name === "Québec" ? "Quebec" : d.properties.name;
                    return selectedStates.some(s => s.name === name);
                })
                .attr("fill", function (d) {
                    if (choroplethData) {
                        const stateName = d.properties.name === "Québec" ? "Quebec" : d.properties.name;
    ...
    useEffect(() => {
        ...
        d3.json("https://code.highcharts.com/mapdata/countries/ca/ca-all.topo.json").then((topo) => {
            const geojson = topojson.feature(topo, topo.objects.default);
            
            // Use fitSize to automatically frame Canada
            ...
            g.selectAll(".state")
                ...
                .on("click", function (event, d) {
                    if (onStateSelectRef.current) {
                        const stateName = d.properties.name === "Québec" ? "Quebec" : d.properties.name;
                        onStateSelectRef.current({ name: stateName });
                    }
                });
            ...
            if (selectedStatesRef.current.length > 0 || choroplethDataRef.current) {
                ...
                g.selectAll(".state")
                    .classed("is-selected", d => {
                        const name = d.properties.name === "Québec" ? "Quebec" : d.properties.name;
                        return selectedStatesRef.current.some(s => s.name === name);
                    })
                    .attr("fill", function (d) {
                        if (cData) {
                            const name = d.properties.name === "Québec" ? "Quebec" : d.properties.name;
                            return cData[name] ? cData[name].color : "#E5E7EB";
                        }
                        return d3.select(this).classed("is-selected") ? "#bfdbfe" : "#EBE9FC";
                    })
                    .attr("stroke", function () {
            ...
            console.error("Failed to load Canada map data:", err);
        });
